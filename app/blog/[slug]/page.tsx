...
const staticArticles: { [key: string]: Article } = {
...
export default function BlogArticlePage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    const getArticle = () => {
      // 1. Chercher d'abord dans les articles dynamiques (localStorage)
      const savedArticles = localStorage.getItem("adminArticles")
      if (savedArticles) {
        const dynamicArticles = JSON.parse(savedArticles)
        const foundDynamic = dynamicArticles.find(
            (a: any) => a.slug === params.slug && a.published !== false
        )
        if (foundDynamic) {
          // Ajouter des tags par défaut si non présents
          if (!foundDynamic.tags) {
            foundDynamic.tags = [foundDynamic.category, "Article", "Droit"]
          }
          return foundDynamic
        }
      }

      // 2. Si pas trouvé, chercher dans les articles statiques
      return staticArticles[params.slug] || null
    }

    const foundArticle = getArticle()
